...
def test_executor_terminates_on_keyword():
    ...
    assert executor._is_termination_msg({"content": "PAYMENT APPROVED. TERMINATE"}) is True
    assert executor._is_termination_msg({"content": "PAYMENT REJECTED. TERMINATE"}) is True
...
def test_parse_payment_request_full():
    from agent_executor import _parse_payment_request
...
async def test_execute_calls_run_payment_assessment():
    ...
    with patch("agent_executor.Part", part_cls), \
         ...
         patch("agent_executor.run_payment_assessment", new_callable=AsyncMock, return_value="APPROVED") as mock_run, \
         patch("agent_executor.new_agent_text_message", return_value="event") as mock_msg:
        await exec_.execute(ctx, event_queue)

    mock_run.assert_called_once_with(
        recipient="alice.fetch", amount=50.0, reason="report",
        llm_config=TEST_LLM, currency="USDC",
    )
...
async def test_execute_handles_error():
    ...
    with patch("agent_executor.Part", part_cls), \
         patch("agent_executor.TextPart", text_part_cls), \
         patch("agent_executor.run_payment_assessment", new_callable=AsyncMock, side_effect=RuntimeError("LLM timeout")), \
         patch("agent_executor.new_agent_text_message", return_value="err") as mock_msg:
        await exec_.execute(ctx, event_queue)
