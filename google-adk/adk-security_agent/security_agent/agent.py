...
from app_setup import agent as security_agent
...
# Initialize uAgent
agent = Agent(name=UAGENT_NAME, seed=UAGENT_NAME, port=8008, mailbox=True)
...
def initialize_adk_agent():
    ...
    adk_runner = Runner(
        agent=security_agent, app_name=APP_NAME, session_service=session_service
    )
...
async def _ensure_session_exists(user_id: str = USER_ID, session_id: str = SESSION_ID):
    ...
    service = (
        adk_runner.session_service
        if adk_runner and hasattr(adk_runner, "session_service")
    ...
    try:
        ...
        try:
            ...
            return (
                ...
                if response_parts
                else "No response from security agent"
            )
        finally:
...
async def startup_handler(ctx: Context):
    ...
    try:
        initialize_adk_agent()
        ctx.logger.info(
            "ADK security agent with guardrails and plugins initialized successfully"
        )

        # Check Model Armor configuration
        model_armor_template = os.getenv(
            "MODEL_ARMOR_TEMPLATE", "security_guardrails_template"
        )
        project_id = os.getenv("GOOGLE_CLOUD_PROJECT", "fetch-ai-innovation-lab")
        location = os.getenv("MODEL_ARMOR_LOCATION", "us-central1")

        ctx.logger.info(
            f"Model Armor Template: projects/{project_id}/locations/{location}/templates/{model_armor_template}"
        )
        ctx.logger.info(
            "Note: Model Armor applies automatically at infrastructure level if configured"
        )
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    try:
        ...
        if content_key in processed_content:
            ...
            if time_diff < 5.0:
                ctx.logger.debug(
                    f"Duplicate message ignored (by content): {text_content[:50]}..."
                )
                return
        ...
        if len(processed_content) > 100:
            ...
            processed_content = {
                k: v for k, v in processed_content.items() if v > cutoff_time
            }
        ...
        ack = ChatAcknowledgement(
            timestamp=datetime.datetime.utcnow(), acknowledged_msg_id=msg.msg_id
        )
        ...
        if stored_identity:
            ...
            ctx.logger.info(
                f"User identified from storage: {user_state.get('user_name')} ({user_role})"
            )
            ...
            if identity_query in valid_roles:
                ...
                confirmation = ChatMessage(
                    ...
                    content=[
                        TextContent(
                            type="text",
                            text=f"✅ Thank you! Your identity has been saved as: **{user_name}**. How can I help you today?",
                        )
                    ],
                )
                ...
                identity_prompt = ChatMessage(
                    ...
                    content=[
                        TextContent(
                            type="text",
                            text="👋 Welcome! Please identify yourself by replying with **one** of the following:\n\n• **student**\n• **developer**\n• **enterprise**\n• **business**\n• **robot**\n\nThis will help me provide you with the appropriate access and assistance.",
                        )
                    ],
                )
