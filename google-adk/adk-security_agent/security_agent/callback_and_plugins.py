...
async def callback_before_agent(
    ...
    _user_name = state.get("user_name")  # noqa: F841
    ...
    if not user_role:
        ...
        if student_profile is False:
            ...
            return types.Content(
                role="model",
                parts=[
                    types.Part(text="Error: User profile not found. Access denied.")
                ],
            )
    ...
    if user_role == "student":
        ...
        return types.Content(
            ...
            parts=[
                types.Part(
                    text=f"Error: Unknown user role '{user_role}'. Access denied for security reasons."
                )
            ],
        )
        ...
        if required_perms:
            ...
            if missing_perms:
                ...
                return types.Content(
                    ...
                    parts=[
                        types.Part(
                            text=f"Error: User profile incomplete. Missing required permissions: {', '.join(missing_perms)}. Access denied."
                        )
                    ],
                )
        ...
        if user_role == "student":
            ...
            if has_restricted:
                return types.Content(
                    ...
                    parts=[
                        types.Part(
                            text="Error: Permission escalation detected. Students cannot have admin privileges. Access denied."
                        )
                    ],
                )
    ...
    if student_profile is False:  # Explicitly False means blocked
        return types.Content(
            role="model",
            parts=[
                types.Part(text="Error: Cannot find student profile. Access denied.")
            ],
        )
