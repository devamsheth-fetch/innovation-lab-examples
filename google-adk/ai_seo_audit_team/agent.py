...
# Initialize uAgent
agent = Agent(name=UAGENT_NAME, seed=UAGENT_NAME, port=8007, mailbox=True)
...
def initialize_adk_agent():
    ...
    adk_runner = Runner(
        agent=seo_audit_team, app_name=APP_NAME, session_service=session_service
    )
...
async def _ensure_session_exists(
    user_id: str, session_id: str, runner_session_service: Any
):
...
class StageTracker:
    ...
    def _detect_from_text(self, text: str) -> tuple[str | None, str | None]:
        ...
        explicit_start_markers = {
            "Page Auditor": [
                "starting page audit",
                "beginning page audit",
                "scraping page",
            ],
            "SERP Analyst": [
                "starting serp analysis",
                "beginning serp analysis",
                "analyzing serp",
            ],
                ...
                if markers and stage_name not in self.completed_stages:
                    if any(marker in text_lower for marker in markers):
                        ...
                        if stage_name != self.last_notified_complete:
                            ...
                            stage_emoji = stage.get("emoji", "")
                            complete_notification = (
                                f"{stage_emoji} **{stage_name} Complete**"
                            )
                            break
...
def _is_audit_request(query: str) -> bool:
    ...
    if any(keyword in query_lower for keyword in audit_keywords):
        ...
        if any(gq in query_lower for gq in general_questions) and not re.search(
            url_pattern, query, re.IGNORECASE
        ):
            return False
    return False
...
async def _process_general_query(
    query: str, sender: str, session_id: str, ctx: Context
) -> str:
    ...
    try:
        ...
        try:
            await general_session_service.create_session(
                app_name=f"{APP_NAME}_general", user_id=sender, session_id=session_id
            )
        except Exception:
            try:
        ...
        async for event in general_query_runner.run_async(
            user_id=sender, session_id=session_id, new_message=content
        ):
        ...
        return (
            ...
            if response_parts
            else "I'm here to help! How can I assist you today?"
        )

    except Exception:
        return "Hello! I'm the Google SEO Audit Team agent. I help analyze websites for SEO optimization. To get started, provide me with a URL to audit (e.g., 'Audit https://example.com')."
...
async def process_with_adk(query: str, sender: str, ctx: Context) -> str:
    ...
    try:
        await _ensure_session_exists(
            sender, str(ctx.session), adk_runner.session_service
        )
        ...
        ctx.logger.info(
            f"🔍 Starting SEO audit for URL: {target_url} | Session ID: {session_id}"
        )
        ...
        async for event in adk_runner.run_async(
            user_id=sender, session_id=session_id, new_message=new_message
        ):
            try:
                ...
                if tool_info and tool_info.get("tool_name") == "firecrawl_scrape":
                    url_arg = (
                        tool_info.get("args", {}).get("url", "unknown")
                        if tool_info.get("args")
                        else "unknown"
                    )
                    ctx.logger.info(
                        f"🔥 Firecrawl: Scraping URL -> {url_arg} | Session ID: {session_id}"
                    )
                    await send_progress_message(
                        ctx, sender, "⏳ Processing... Scraping page content..."
                    )
                ...
                if tool_info and tool_info.get("tool_name"):
                    ...
                    if "google_search" in tool_name:
                        ...
                        search_query = args.get("query") or args.get(
                            "request", "unknown"
                        )
                        ctx.logger.info(
                            f"🔎 Google Search: Query -> {search_query} | Session ID: {session_id}"
                        )
                        await send_progress_message(
                            ctx, sender, "⏳ Processing... Analyzing SERP data..."
                        )
                ...
                start_notification, complete_notification = tracker.process_event(
                    event, text
                )
                ...
                if current_agent and current_agent != last_agent and last_agent:
                    ...
                    if prev_stage:
                        ...
                        if prev_name and prev_name not in tracker.completed_stages:
                            ...
                            ctx.logger.info(
                                f"✅{prev_name}completed | Session ID: {session_id}"
                            )
                            await send_progress_message(
                                ctx,
                                sender,
                                f"⏳ Processing... {prev_emoji} {prev_name} complete",
                            )
                ...
                if start_notification:
                    ...
                    await send_progress_message(
                        ctx,
                        sender,
                        f"⏳ Processing... {start_notification.replace('**', '')}",
                    )
                ...
                if text and not _is_json_content(text):
                    ...
                    if (
                        text.strip().startswith("# SEO Audit Report")
                        or current_agent == "OptimizationAdvisorAgent"
                    ):
                ...
                ctx.logger.error(
                    f"Error processing event: {str(e)} | Session ID: {session_id}",
                    exc_info=True,
                )
        ...
        if last_agent:
            ...
            if final_stage and final_stage["name"] not in tracker.completed_stages:
                ...
                # Clean up logging - remove extra spaces
                ctx.logger.info(
                    f"✅{final_stage['name']}completed | Session ID: {session_id}"
                )
                await send_progress_message(
                    ctx,
                    sender,
                    f"⏳ Processing... {final_stage['emoji']} {final_stage['name']} complete",
                )
        ...
        full_response = (
            " ".join(str(part) for part in response_parts if part).strip()
            if response_parts
            else ""
        )
        ...
        if final_response and final_response.strip():
            ctx.logger.info(
                f"✅ SEO audit completed for URL: {target_url} | Session ID: {session_id}"
            )
            await send_progress_message(ctx, sender, final_response)
            return final_response
        else:
            ctx.logger.warning(
                f"⚠️ No report generated for URL: {target_url} | Session ID: {session_id}"
            )
...
async def startup_handler(ctx: Context):
    ...
    try:
        ...
        initialize_general_query_agent()
        ctx.logger.info(
            "ADK SEO audit team and general query agent initialized successfully"
        )
    except Exception as e:
...
async def handle_message(ctx: Context, sender: str, msg: ChatMessage):
    ...
    try:
        ...
        await process_with_adk(text_content, sender, ctx)
