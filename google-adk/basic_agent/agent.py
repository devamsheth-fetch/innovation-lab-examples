...
# Initialize uAgent
agent = Agent(name=APP_NAME, seed=APP_NAME, port=8005, mailbox=True)
...
async def _ensure_session_exists():
    ...
    try:
        await runner_session_service.create_session(
            app_name=APP_NAME, user_id=FIXED_USER_ID, session_id=FIXED_SESSION_ID
        )
    except Exception:
        try:
...
async def process_with_adk(query: str) -> str:
    ...
    try:
        ...
        async for event in adk_runner.run_async(
            user_id=FIXED_USER_ID, session_id=FIXED_SESSION_ID, new_message=new_message
        ):
