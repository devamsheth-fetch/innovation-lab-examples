...
def initialize_adk_agent():
    ...
    adk_runner = Runner(app=app, session_service=session_service)
...
async def process_with_adk(query: str, session_id: str = SESSION_ID) -> str:
    ...
    try:
                            ...
                            for part in event.content.parts:
                                if hasattr(part, "text") and part.text:
                                    if part.text not in response_parts:
                                        ...
                                        logger.debug(
                                            f"Added text from final response: {part.text[:100]}"
                                        )
        ...
        logger.debug(
            f"Total events received: {event_count}, Response parts: {len(response_parts)}"
        )

        raw_response = (
            " ".join(response_parts).strip()
            if response_parts
            else "No response from ADK agent"
        )
    ...
    try:
        ...
        if content_key in processed_content:
            ...
            if time_diff < 5.0:  # Same content from same sender within 5 seconds
                ctx.logger.debug(
                    f"Duplicate message ignored (by content): {text_content[:50]}... (received {time_diff:.2f}s ago)"
                )
                return
